...
class DatasetFSS(Dataset):
    def __init__(
        self, datapath, fold, transform, split, shot, use_original_imgsize=None
    ):
        self.split = split
        self.benchmark = "fss"
        self.shot = shot
    ...
    def load_frame(self, query_name, support_names):
        ...
        if self.shot:
            ...
            support_names = [
                os.path.join(os.path.dirname(name), sid) + ".png"
    ...
    def sample_episode(self, idx):
        ...
        if self.split == "test" and self.shot == 1:
            while True:
                support_name = 1
                support_name = (
                    os.path.join(os.path.dirname(query_name), str(support_name))
                    + ".jpg"
                )
    ...
    def build_img_metadata(self):
        ...
        for cat in self.categories:
            img_paths = sorted(
                [path for path in glob.glob("%s/*" % os.path.join(self.base_path, cat))]
            )
            if self.split == "test" and self.shot == 1:
