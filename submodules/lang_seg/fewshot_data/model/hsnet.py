...
class HypercorrSqueezeNetwork(nn.Module):
    def __init__(self, backbone, use_original_imgsize):
        ...
        self.stack_ids = (
            torch.tensor(self.lids).bincount().__reversed__().cumsum(dim=0)[:3]
        )
        self.backbone.eval()
    ...
    def forward(self, query_img, support_img, support_mask):
        with torch.no_grad():
            query_feats = self.extract_feats(
                query_img, self.backbone, self.feat_ids, self.bottleneck_ids, self.lids
            )
            ...
            corr = Correlation.multilayer_correlation(
                query_feats, support_feats, self.stack_ids
            )

        logit_mask = self.hpn_learner(corr)
        if not self.use_original_imgsize:
            logit_mask = F.interpolate(
                logit_mask, support_img.size()[2:], mode="bilinear", align_corners=True
            )
    ...
    def predict_mask_nshot(self, batch, nshot):
        ...
        for s_idx in range(nshot):
            ...
            if self.use_original_imgsize:
                ...
                logit_mask = F.interpolate(
                    logit_mask, org_qry_imsize, mode="bilinear", align_corners=True
                )
