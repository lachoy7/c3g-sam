...
class Visualizer:
    ...
    def visualize_prediction_batch(
        ...
        for sample_idx, (
            ...
            zip(spt_img_b, spt_mask_b, qry_img_b, qry_mask_b, pred_mask_b, cls_id_b)
        ):
    ...
    def visualize_prediction(
        ...
        spt_masked_pils = [
            Image.fromarray(cls.apply_mask(spt_img, spt_mask, spt_color))
            for spt_img, spt_mask in zip(spt_imgs, spt_masks)
        ]
        ...
        pred_masked_pil = Image.fromarray(
            cls.apply_mask(
                qry_img.astype(np.uint8), pred_mask.astype(np.uint8), pred_color
            )
        )
        qry_masked_pil = Image.fromarray(
            cls.apply_mask(
                qry_img.astype(np.uint8), qry_mask.astype(np.uint8), qry_color
            )
        )

        merged_pil = cls.merge_image_pair(
            spt_masked_pils + [pred_masked_pil, qry_masked_pil]
        )

        iou = iou.item() if iou else 0.0
        merged_pil.save(
            cls.vis_path
            + "%d_%d_class-%d_iou-%.2f" % (batch_idx, sample_idx, cls_id, iou)
            + ".jpg"
        )
