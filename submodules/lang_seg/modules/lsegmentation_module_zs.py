...
class LSegmentationModuleZS(pl.LightningModule):
    def __init__(self, data_path, dataset, batch_size, base_lr, max_epochs, **kwargs):
        ...
        self.fixed_encoder = (
            True if kwargs["use_pretrained"] in ["clip_fixed"] else False
        )
        ...
        if kwargs["backbone"] in ["clip_resnet101"]:
            ...
            FSSDataset.initialize(
                img_size=480, datapath=self.args.datapath, use_original_imgsize=False
            )
    ...
    def training_step(self, batch, batch_nb):
        if self.args.finetune_mode:
            if self.args.nshot == 5:
                ...
                if (
                    self.args.benchmark == "pascal"
                    and batch["support_ignore_idxs"] is not None
                ):
                    query_ignore_idx = batch["support_ignore_idxs"].view(
                        -1, bshape[3], bshape[4]
                    )
                    area_inter, area_union = Evaluator.classify_prediction(
                        out.argmax(dim=1), target, query_ignore_idx
                    )
                else:
                    area_inter, area_union = Evaluator.classify_prediction(
                        out.argmax(dim=1), target
                    )
            else:
            ...
            img = torch.cat(
                [batch["support_imgs"].squeeze(1), batch["query_img"]], dim=0
            )
            target = torch.cat(
                [batch["support_masks"].squeeze(1), batch["query_mask"]], dim=0
            )
            ...
            if (
                self.args.benchmark == "pascal"
                and batch["query_ignore_idx"] is not None
            ):
                query_ignore_idx = torch.cat(
                    [
                        batch["support_ignore_idxs"].squeeze(1),
                        batch["query_ignore_idx"],
                    ],
                    dim=0,
                )
                area_inter, area_union = Evaluator.classify_prediction(
        ...
        self.train_average_meter.update(
            area_inter, area_union, class_info, loss.detach().clone()
        )
        if self.global_rank == 0:
            return_value = self.train_average_meter.write_process(
                batch_nb,
    ...
    def training_epoch_end(self, outs):
        ...
        self.fewshot_trn_miou, self.fewshot_trn_fb_iou = (
            self.train_average_meter.compute_iou()
        )
    ...
    def validation_step(self, batch, batch_nb):
        if self.args.finetune_mode and self.args.nshot == 5:
            ...
            if (
                ...
                query_ignore_idx = batch["query_ignore_idx"].view(
                    -1, bshape[3], bshape[4]
                )
                area_inter, area_union = Evaluator.classify_prediction(
                    out.argmax(dim=1), target, query_ignore_idx
        ...
        self.val_average_meter.update(
            area_inter, area_union, class_info, val_loss.detach().clone()
        )
        if self.global_rank == 0:
            return_value = self.val_average_meter.write_process(
                batch_nb,
    ...
    def validation_epoch_end(self, outs):
        ...
        if self.global_rank == 0:
            Logger.tbd_writer.add_scalars(
                "fewshot_data/data/loss",
                {"trn_loss": self.fewshot_trn_loss, "val_loss": val_loss},
                self.current_epoch,
            )
            Logger.tbd_writer.add_scalars(
                "fewshot_data/data/miou",
                {"trn_miou": self.fewshot_trn_miou, "val_miou": val_miou},
                self.current_epoch,
            )
            Logger.tbd_writer.add_scalars(
                "fewshot_data/data/fb_iou",
                {"trn_fb_iou": self.fewshot_trn_fb_iou, "val_fb_iou": val_fb_iou},
                self.current_epoch,
            )
        ...
        threshold_epoch = 3
        if (
            self.args.benchmark in ["pascal", "coco"]
            and self.current_epoch >= threshold_epoch
        ):
            print("End this loop!")
            exit()
    ...
    def configure_optimizers(self):
        ...
        if self.fixed_encoder:
            ...
            params_list.append(
                {
                    "params": self.net.pretrained.act_postprocess1.parameters(),
                    "lr": self.base_lr,
                }
            )
            params_list.append(
                {
                    "params": self.net.pretrained.act_postprocess2.parameters(),
                    "lr": self.base_lr,
                }
            )
            params_list.append(
                {
                    "params": self.net.pretrained.act_postprocess3.parameters(),
                    "lr": self.base_lr,
                }
            )
            params_list.append(
                {
                    "params": self.net.pretrained.act_postprocess4.parameters(),
                    "lr": self.base_lr,
                }
            )
        else:
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument(
            "--data_path", type=str, default="", help="path where dataset is stored"
        )
        parser.add_argument(
            "--dataset",
            type=str,
