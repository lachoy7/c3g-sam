"""Replica distillation dataset loader (pre-computed SAM features).
...
Reuses view sampling, camera loading, image preprocessing, and baseline
normalization from :mod:`dataset_replica_2dseg`.  Produces ``sam_features``
...
@dataclass
class ReplicaDistillCfg(DatasetCfgCommon):
    name: str
    roots: list[Path]
    scenes: list[str]
    baseline_min: float
...
@dataclass
class DatasetReplicaDistillCfgWrapper:
    replica_distill: ReplicaDistillCfg
...
class DatasetReplicaDistill(IterableDataset):
    ...
    def __init__(self, cfg: ReplicaDistillCfg, stage: str, view_sampler: ViewSampler):
        super().__init__()
        self.cfg = cfg
        self.stage = stage
        ...
        for scene in scene_list:
            ...
            for target_idx in target_indices:
                ...
                for view_index in idxs:
                    ...
                    # Check that image and SAM feature files exist
                    if not paths.image.is_file():
                        logger.warning(f"Missing image for {scene} frame {frame_id}")
                        valid = False
                        break
                    ...
                    # Load pre-computed SAM features
                    sam_feat = torch.load(sam_path, map_location="cpu")
                    if sam_feat.shape != (256, 64, 64):
                        logger.warning(
                            f"Unexpected SAM feature shape {sam_feat.shape} for "
...
DATASET_CLASS = DatasetReplicaDistill
DATASET_NAMES = ("replica_distill",)
CFG_WRAPPERS = (DatasetReplicaDistillCfgWrapper,)
