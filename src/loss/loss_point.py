...
class Regr3D(nn.Module):
    ...
    def forward(
        ...
        if dist_clip is not None:
            ...
            quantiles_1 = torch.quantile(
                dis1_flat, torch.tensor([0.002, 0.998]).to(dis1_flat.device), dim=1
            )
            quantiles_2 = torch.quantile(
                dis2_flat, torch.tensor([0.002, 0.998]).to(dis2_flat.device), dim=1
            )

            # Create masks based on the quantiles
            valid1 = (dis1 >= quantiles_1[0].view(-1, 1, 1)) & (
                dis1 <= quantiles_1[1].view(-1, 1, 1)
            )
            valid2 = (dis2 >= quantiles_2[0].view(-1, 1, 1)) & (
                dis2 <= quantiles_2[1].view(-1, 1, 1)
            )
