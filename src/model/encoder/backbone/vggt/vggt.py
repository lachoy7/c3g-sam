...
class VGGT(nn.Module, PyTorchModelHubMixin):
    ...
    def forward(
        ...
        elif self.track_head is not None:
            predictions["feature"] = self.track_head.feature_extractor(
                aggregated_tokens_list, images, patch_start_idx
            )
            predictions["feature"] = self.track_head.tracker.fmap_norm(
                predictions["feature"].permute(0, 1, 3, 4, 2)
            )
            predictions["feature"] = predictions["feature"].permute(0, 1, 4, 2, 3)
