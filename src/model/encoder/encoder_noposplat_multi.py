...
from .heads import head_factory, DPTHead, GSDPTHead
    ...
    def forward(
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            if self.dpt_head is not None:
                ...
                all_mean_res = [{"pts3d": res[0][:, i]} for i in range(v)]

            else:
                res1 = self._downstream_head(
                    1, [tok[:, 0].float() for tok in dec_feat], shape[:, 0]
                )
            ...
            if self.dpt_gs_head is not None:
                ...
                all_other_params = [
                    rearrange(GS_res[:, i], "b d h w -> b (h w) d") for i in range(v)
                ]
