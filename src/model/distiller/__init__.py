...
def get_distiller(name):
    ...
    if name == "dust3r":
        ...
        weight_path = (
            "./pretrained_weights/MASt3R_ViTLarge_BaseDecoder_512_catmlpdpt_metric.pth"
        )
    else:
        raise NotImplementedError(f"unexpected {name=}")
    ckpt_weights = torch.load(weight_path, map_location="cpu")["model"]
    missing_keys, unexpected_keys = distiller.load_state_dict(
        ckpt_weights, strict=False if name == "mast3r" else True
    )
