...
from ..misc.sam_features import downsample_sam_for_encoder, reorder_context_target
...
class TestCfg:
    ...
    eval_lerf_mask: bool = False
    save_lerf_mask_preds: bool = False
...
class ModelWrapper(LightningModule):
    ...
    def __init__(
        ...
        if dino is not None:
            ...
            self.latent_mean = (
                dino["latent_mean"] if dino["latent_mean"] is not None else 0
            )
            self.latent_var = (
                dino["latent_var"] if dino["latent_var"] is not None else 1
            )
        else:
        ...
        self.per_image_ious = []
        self.lerf_mask_records: list[dict[str, float | str]] = []
        self.lerf_mask_summary: dict = {}
        self.per_image_accs = []
    ...
    def _sam_context_features(
        self, batch: BatchedExample, h: int, w: int
    ) -> Tensor | None:
        if not self.encoder.cfg.feature_dim:
            return None
        precomputed = batch["context"].get("sam_features")
        if precomputed is not None:
            return downsample_sam_for_encoder(
                precomputed.float().to(self.device),
                h,
                w,
                self.encoder.patch_size,
            )
        return self.forward_foundation_model(
            batch["context"]["image"],
            sam_image=batch["context"].get("sam_image"),
        )

    def _sam_merged_features(
        self,
        ...
        *,
        ...
        ctx = batch["context"].get("sam_features")
        tgt = batch["target"].get("sam_features")
        if ctx is not None and tgt is not None:
            merged = torch.cat([ctx, tgt], dim=1).float().to(self.device)
            if interpolate:
                return downsample_sam_for_encoder(
                    merged, h, w, self.encoder.patch_size
                )
            return merged
        return self.forward_foundation_model(
            torch.cat(
    ...
    def _sam_features_for_loss(self, batch: BatchedExample) -> Tensor:
        num_ctx = batch["context"]["image"].shape[1]
    ...
    def _sam_target_features_64(self, batch: BatchedExample) -> Tensor | None:
        ...
        if precomputed is not None:
            return precomputed.float().to(self.device)
    ...
    def _maybe_log_sam_debug_visualizations(
        ...
        *,
        ...
        tgt_sam = self._sam_target_features_64(batch)
        if tgt_sam is None:
            return
        ...
        target_sam = tgt_sam[0, 0].detach().float()
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.train_cfg.random_select_context_view:
            if self.global_rank == 0:
                num_ctx_views = torch.randint(
                    2, batch["context"]["extrinsics"].shape[1] + 1, size=(1,)
                ).item()
            else:
                num_ctx_views = 0
        ...
        context_feature = self._sam_context_features(batch, h, w)
        ...
        if self.train_cfg.feature_rendering_loss > 0:
            ...
            gaussian_feature = F.interpolate(
                gaussian_feature.reshape(B * N, -1, H, W),
                size=(FH, FW),
                mode="bilinear",
                align_corners=False,
            ).reshape(B, N, -1, FH, FW)
    ...
    def forward_foundation_model(
        ...
        with torch.no_grad():
            if self.train_cfg.reproj_model == "dinov2":
                ...
                )[0].reshape(B, V, -1, H // 14, W // 14)

            elif "dinov3" in self.train_cfg.reproj_model:
                context_feature = self.dino_model(
                    **self.dino_processor(
                        (input_image.reshape(B * V, C, H, W) + 1) / 2.0 * 255,
                        return_tensors="pt",
                    ).to(self.device)
                )
                context_feature = rearrange(
                    context_feature["last_hidden_state"][:, 5:],
                    "b (h w) c -> b c h w",
                    h=H // 16,
                    w=W // 16,
                )
                ...
                context_feature = self.lseg_feature_extractor.extract_features(
                    input_image.reshape(B * V, 3, H, W)
                )
                ...
                if self.train_cfg.reproj_model == "vggt_tracking":
                    ...
                    aggregated_tokens_list, patch_start_idx = self.vggt(
                        input_image, only_feature=True
                    )
                    vggt_features = aggregated_tokens_list[-1][:, :, patch_start_idx:]
                    context_feature = rearrange(
                        vggt_features, "b n (h w) c -> b n c h w", h=H // 14, w=W // 14
                    )
                ...
                input_image = (input_image - mean[None, None, :, None, None]) / std[
                    None, None, :, None, None
                ]
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.test_cfg.visualize_gaussian_token >= 0:
            ...
            visualize_attention_map(
                ...
                num_heads,
                gaussian_token_idx,
                batch["context"]["image"].shape[3:5],
                patch_size=self.encoder.patch_size,
                output_path=path / scene / f"{gaussian_token_idx}_layer1",
            )
            ...
            visualize_attention_map(
                ...
                batch["context"]["image"].shape[3:5],
                patch_size=self.encoder.patch_size,
                output_path=path / scene / f"{gaussian_token_idx}_layer2",
            )

            C0 = 0.28209479177387814
            gaussians.harmonics[:, gaussian_token_idx, :, 0] = (
        ...
        if self.test_cfg.eval_lerf_mask:
            self._run_lerf_mask_eval(batch, output, path)
        ...
        if self.test_cfg.compute_scores:
            ...
            self.log_dict(
                all_metrics, on_step=True, on_epoch=True, prog_bar=True, logger=True
            )
        ...
        if self.test_cfg.save_compare:
            ...
            if self.encoder.cfg.feature_dim:
                ...
                if (
                    "dinov2" in self.train_cfg.reproj_model
                    or "vggt" in self.train_cfg.reproj_model
                ):
                    B, V, C, H, W = batch["target"]["image"].shape
                    target_image = F.interpolate(
                        batch["target"]["image"].reshape(B * V, C, H, W),
                        size=(224, 224),
                        mode="bilinear",
                        align_corners=False,
                    ).reshape(B, V, C, 224, 224)
                else:
                    target_image = batch["target"]["image"]

                tgt_sam = batch["target"].get("sam_image")
                foundation_feature = self._sam_target_features_64(batch)
                if foundation_feature is None:
                ...
                if (
                    "dino" not in self.train_cfg.reproj_model
                    and "vggt" not in self.train_cfg.reproj_model
                ):
                    pca_images, pca_vggt_images = [], []
                    V = gaussian_feature.shape[1]

                    for i in range(foundation_feature.shape[1]):
                        pca_gaussian_img = run_pca(
                            gaussian_feature[0, i].unsqueeze(dim=0), (H, W)
                        )  # (C, H, W)
                        pca_images.append(pca_gaussian_img.squeeze(dim=0))

                        pca_vggt_img = run_pca(
                            foundation_feature[0, i].unsqueeze(dim=0), (H, W)
                        )  # (C, H, W)
                    ...
                    preds = []
                    targets = []
                    for i, (index, g_upfeat) in enumerate(
                        zip(batch["target"]["index"][0], gaussian_feature)
                    ):
                        if self.test_cfg.forward_vfm:
                            g_upfeat = foundation_feature[i]
                            if "lseg" in self.train_cfg.reproj_model:
                                g_upfeat = (
                                    self.lseg_feature_extractor.scratch.output_conv(
                                        g_upfeat
                                    )
                                )
                        ...
                        if self.train_cfg.reproj_model == "lseg":
                            pred = self.lseg_feature_extractor.decode_feature(
                                g_upfeat, labelset=labelset
                            )
                        else:
                    ...
                    for pred in preds:
                        for index, seg_pred in zip(batch["target"]["index"][0], pred):
                            ...
                            seg_pred_vis = save_segmap(
                                gaussian_feature,
                                seg_pred,
                                index,
                                save_dir,
                                labels,
                                self.test_cfg.color_hex_list,
                            )
                    ...
                    for i in range(len(cocnat_pca_images)):
                        save_image(
                            cocnat_pca_images[i], path / f"{scene}_cocnat_pca{i}.png"
                        )
    ...
    def on_test_epoch_end(self):
        mean_iou = (
            sum(self.per_image_ious) / len(self.per_image_ious)
            if self.per_image_ious
            else 0.0
        )
        mean_acc = (
            sum(self.per_image_accs) / len(self.per_image_accs)
            if self.per_image_accs
            else 0.0
        )
    ...
    def test_step_align(self, batch, gaussians, verbose=False):
        ...
        with torch.set_grad_enabled(True):
            cam_rot_delta = nn.Parameter(
                torch.zeros([b, v, 3], requires_grad=True, device=self.device)
            )
                ...
                for i in logger:
                    ...
                    if verbose:
                        logger.set_description(
                            f"pose optim step {i}; loss = {total_loss:.6f}"
                        )
                    ...
                    with torch.no_grad():
                        ...
                        extrinsics = rearrange(
                            new_extrinsic, "(b v) i j -> b v i j", b=b, v=v
                        )
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if self.train_cfg.feature_rendering_loss > 0:
            ...
            gaussian_feature = torch.cat(
                (context_gaussian_feature, gaussian_feature), dim=1
            )

            foundation_feature = self._sam_merged_features(batch, h, w, interpolate=True)
            context_foundation_features = self._sam_context_features(batch, h, w)
            ...
            for i in range(context_foundation_features.shape[1]):
                pca_context_img = run_pca(
                    context_foundation_features[0, i].unsqueeze(dim=0), (H, W)
                )  # (C, H, W)
    ...
    def render_video_generic(
        ...
        context_feature = (
            self._sam_context_features(batch, h, w)
            if self.encoder.cfg.feature_dim
            else None
        )
