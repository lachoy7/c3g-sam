...
class AttentionPool2d(nn.Module):
    ...
    def forward_v(self, x: torch.Tensor):
        ...
        interpolated_pe = interpolate_positional_embedding(
            self.positional_embedding, x.permute(1, 0, 2), patch_size=1, w=w, h=h
        )
...
class ResidualAttentionBlock(nn.Module):
    def __init__(
        self, d_model: int, n_head: int, attn_mask: torch.Tensor | None = None
    ):
        super().__init__()
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, patch_output: bool = False):
        ...
        x = x + interpolate_positional_embedding(
            self.positional_embedding, x, patch_size=self.patch_size, w=w, h=h
        )
        x = self.ln_pre(x)
